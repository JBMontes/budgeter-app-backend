...
transactions.get("/:id", (req, res, next) => {
  ...
  const purchase = transactionArray.find(
    (transaction) => parseInt(transaction.id) === parseInt(id)
  );
...
transactions.put("/:id", (req, res) => {
  const { id } = req.params;
  const foundIndex = transactionArray.findIndex((element) => parseInt(element.id) === parseInt(id));
  transactionArray[foundIndex] = req.body;
